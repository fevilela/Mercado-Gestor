...
const expensesData = [
  { name: "Fornecedores", value: 45000 },
  { name: "Pessoal", value: 15000 },
    ...
    <Layout>
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-heading font-bold tracking-tight text-foreground">Financeiro</h1>
            <p className="text-muted-foreground">Fluxo de caixa, contas a pagar e DRE.</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="bg-emerald-500/10 border-emerald-500/20">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-emerald-700 dark:text-emerald-400">Receitas (Mês)</CardTitle>
              <ArrowUpRight className="h-4 w-4 text-emerald-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-400">R$ 142.350,00</div>
          ...
          <Card className="bg-rose-500/10 border-rose-500/20">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-rose-700 dark:text-rose-400">Despesas (Mês)</CardTitle>
              <ArrowDownRight className="h-4 w-4 text-rose-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-rose-700 dark:text-rose-400">R$ 89.420,00</div>
          ...
          <Card className="bg-primary/10 border-primary/20">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-primary">Saldo Líquido</CardTitle>
              <DollarSign className="h-4 w-4 text-primary" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-primary">R$ 52.930,00</div>
                ...
                <CardContent className="pl-2">
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={cashFlowData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="in" name="Entradas" stroke="#10b981" strokeWidth={2} dot={false} />
                      <Line type="monotone" dataKey="out" name="Saídas" stroke="#ef4444" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {[
                    { id: 1, desc: "Fornecedor Atacadista A", due: "Hoje", value: "R$ 4.500,00", status: "Pendente" },
                    { id: 2, desc: "Conta de Energia (Enel)", due: "Amanhã", value: "R$ 1.250,00", status: "Agendado" },
                    { id: 3, desc: "Aluguel Loja", due: "15/12", value: "R$ 3.000,00", status: "Pendente" },
                    { id: 4, desc: "Internet Fibra", due: "15/12", value: "R$ 150,00", status: "Pendente" },
                  ].map((item) => (
                    <div key={item.id} className="flex items-center justify-between border-b border-border pb-4 last:border-0 last:pb-0">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Button size="sm" variant={item.status === 'Agendado' ? 'secondary' : 'default'}>
